	...
	let login =()=>{
		...
		//	type : 'submit',
			...
			click : e=>{
				e.preventDefault()
				$.ajax({
					url : _+'/customers/' + $('#cemail').val() + '/login',
					type : 'POST',
					dataType : 'json',
					data : JSON.stringify({ 
						cemail : $('#cemail').val(), 
		...
		$('#a_go_join').click(e=>{
			...
			$('#cemail').keyup(()=>{
				if($('#cemail').val().length > 2){
					$.ajax({
						url : _+'/customers/'+$('#cemail').val()+'/exist',
						contentType : 'application/json',
						success : d => {
...
//					type : 'submit',
	...
	let join =()=>{
		...
		//		reg : $('#reg').val()
			...
			contentType : 'application/json',
			success : d => {	// sender, d가 자바에서 map, d.cemail map의 키값
