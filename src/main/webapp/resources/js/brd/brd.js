...
/*		$.ajax({
	...
	//		let i = 0
			...
			$.each(d.articles, (i, j)=>{
				...
				$('<strong class="d-block text-gray-dark">@<a>'+ j.cemail +'</a></strong>') // .appendTo면 뒤로 붙음, 앞에붙이면 .prependTo()
				.appendTo("#id_"+i)
				.click(()=>{
					alert('아이디 클릭')
				})
...
/*		sessionStorage.setItem('ctx', '/web')
	...
	//	$('#recent_updates').html(brd_vue.brd_write())// $cemail ))
...
//		$('#recent_updates div.container-fluid').html('<h1>ARTICLE DETAIL</h1>') - 하면 id, title, content 칸 모두 없어짐
		...
		.click(e=>{
			...
			alert(json.cemail + json.title +json.content)
			$.ajax({
